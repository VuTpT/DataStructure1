package funix.vn.main;

import java.io.IOException;
import java.util.Arrays;
import java.util.Scanner;

import funix.vn.argorithm.Argorithm;
import funix.vn.utils.Constant;
import funix.vn.utils.IOReadWriteFile;
import funix.vn.utils.RandomArray;
import funix.vn.utils.Validator;
...
public class Main {
	...
	static Argorithm argorithm = new Argorithm();
	...
	private static void feature3() {
		...
		if (!isExist) {
			...
			tempArray = argorithm.ascBubbleSort(fileName, array);
	...
	private static void feature4() {
		...
		if (!isExist) {
			...
			tempArray = argorithm.ascSelectionSort(fileName, array);
	...
	private static void feature5() {
		...
		if (!isExist) {
			...
			tempArray = argorithm.ascInsertionSort(fileName, array);
	...
	private static void feature6(Scanner scanner) {
		...
		if (!isExist) {
			...
			if (isDelete) {
				...
				argorithm.linearSearch(fileName, array, value);
			}
		}
	}
	...
	private static void feature7(Scanner scanner) {
		...
		if (!isExist) {
			...
			if (tempArray != null) {
				...
				number = argorithm.binarySearch(tempArray, 0, right, value);
	...
	private static void feature8() {
		...
		argorithm.ascBubbleSort(null, array1);
		end = System.nanoTime();
		total = (int) (end - start);
		System.out.println("\nTotal: " + total);
		...
		System.out.println();
		start = System.nanoTime();
		argorithm.ascBubbleSort(null, array2);
		end = System.nanoTime();
		total = (int) (end - start);
		...
		System.out.println();
		start = System.nanoTime();
		argorithm.ascBubbleSort(null, array3);
		end = System.nanoTime();
		total = (int) (end - start);
		System.out.println("\nTotal: " + total);
